...
import eu.sia.meda.error.condition.ErrorManagerNotEnabledCondition;
...
import eu.sia.meda.exceptions.CircuitBreakerException;
...
import eu.sia.meda.util.LogMaskUtils;
...
/**
 * The Class OldMedaExceptionManager.
 */
@ControllerAdvice
@Conditional({ErrorManagerNotEnabledCondition.class})
public class OldMedaExceptionManager extends ResponseEntityExceptionHandler {
   ...
   @ResponseBody
   private MicroServiceExceptionResponse handleCircuitBreakerException(Exception e, HttpServletRequest request, HttpServletResponse response) {
      MedaError error = this.generateError(e, HttpStatus.SERVICE_UNAVAILABLE, request);
      return new MicroServiceExceptionResponse(error);
   }
   ...
   private Map<String, Object> addDefaultTechnicalInfos(Exception e, HttpStatus status, @Nullable HttpServletRequest request) {
      ...
      if (e.getMessage() != null) {
         technicalInfo.put("message", LogMaskUtils.maskFields(e.getMessage()));
      } else {
