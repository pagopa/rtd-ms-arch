 ...
 */
public abstract class BaseEventConnector<INPUT, OUTPUT, DTO, RESOURCE>
	...
	@PostConstruct
	private void init() {
		ArchEventConfigurationService.EventConfiguration eventConfiguration = this.configuration
				.retrieveEventConfiguration(this.getClass().getSimpleName());
		if (eventConfiguration == null) {
			...
			this.setSaslMechanism(eventConfiguration.getSaslMechanism());
			...
			this.setRetries(eventConfiguration.getRetries());
			this.setConnectionMaxIdleMs(eventConfiguration.getConnectionMaxIdleMs());
	...
	protected EventResponse doExecute(EventRequest<DTO> request) {
		if (request.getPayload() == null) {
			...
			if (logger.isDebugEnabled()) {
				logger.debug(String.format("Sending message on topic '%s' towards bootstrap servers '%s'", topicTmp,
						this.bootstrapServers));
			}

			return new EventResponse(true, SUCCESS,
					this.producer.send(topicTmp, request.getKey(), request.getPayload(), request.getHeaders()));
		}
	}
	 ...
	 * @return the output
	 */
	public final OUTPUT call(INPUT input, IEventRequestTransformer<INPUT, DTO> requestTransformer,
                             IEventResponseTransformer<RESOURCE, OUTPUT> responseTransformer, Object... args) {
	...
	private EventProducer getProducer() {
		...
		if (this.retries != null) {
			configProps.put("retries", this.retries);
		}
		...
		if (this.connectionMaxIdleMs != null) {
			configProps.put("connections.max.idle.ms", this.connectionMaxIdleMs);
		}
	...
	/*	ProducerFactory<String, byte[]> producerFactory =  new TracingProducerFactory<> (new DefaultKafkaProducerFactory<>(configProps),tracer);
		KafkaTemplate<String, byte[]> kafkaTemplate = new KafkaTemplate<>(producerFactory);
		return new EventProducerImpl(kafkaTemplate);*/
	}
